...
export function FTable(word_8b) {
    var row = word_8b >>> 4,
        col = word_8b & 15;

    return f_table[row * 16 + col];
}
...
export function subkeys(splitKey, isDecrypt) {
    var keyList = [],
        roundKey = Object.assign({}, splitKey); // copy splitKey to preserve the original

    // generate all 192 subkeys
    for (var r = 0; r < 16; r++) {
        let sublist = [];
        // each round uses 12 subkeys
        for (var _ = 0; _ < 3; _++) {
            for (var i = 0; i < 4; i++) {
                sublist.push( subkey(roundKey, 4 * r + i) );
            }
        }
        keyList.push(sublist);
    }

    // reverse order of subkeys if decrypting
    if (isDecrypt) keyList.reverse();
    
    // flatten from: [ ... [ ... subkeys ] ] to: [ ... subkeys ]
    keyList = keyList.reduce((keys, sublist) => keys.concat(sublist));

    // setup iterator
    r = 0;
    keyList.next = () => {
        if (r === keyList.length) r = 0;
        return keyList[r++];
    }
...
export function parseKey(key) {
    return {
        left: parseInt(key.substring(0, 8), 16),
        right: parseInt(key.substring(8), 16)
    };
}
...
function G(word_16b, subkeys) {
    var g0 = word_16b >>> 8,
        ...
        console.log(`g0: ${g0.toString(16)} g1: ${g1.toString(16)} g2: ${g2.toString(16)} g3: ${g3.toString(16)} g4: ${g4.toString(16)} g5: ${g5.toString(16)} `)
...
function encryptBlock(source, index, key, subkeys) {
    var temp,
        text = source.slice(index, index + 4),
        log = text,
        block = readBlock64(source, index);

    log += " => " + block.left.toString(16) + " " + block.right.toString(16);

    console.log("FUCK!");

    console.log("input: " + block.left.toString(16) + " " + block.right.toString(16));
    ...
    log += " => " + block.left.toString(16) + " " + block.right.toString(16);
    // console.log("whitened: " + block.left.toString(16) + " " + block.right.toString(16));
...
function WSUCrypt(plaintext, key, isDecrypt) {
    ...
    var ciphertext = "",
        ...
        keys = subkeys(splitKey, isDecrypt); // generate all the subkeys

    // encrypt plaintext block by block
    const len = plaintext.byteLength / 4;
    for (var i = 0; i < len; i++) {
        ciphertext += encryptBlock(plaintext, i * 4, splitKey, keys );
    }
