...
describe('block32 functions', () => {
    test('it can read 32 bit blocks', () => {
        expect(util.readBlock32('abcdefgh', 0)).toBe(1633837924);
        expect(util.readBlock32('abcdefgh', 4)).toBe(1701209960);
    })
    test('it can write 32 bit blocks back as strings', () => {
        expect(util.block32ToString(1633837924)).toBe('abcd');
        expect(util.block32ToString(1701209960)).toBe('efgh');
    })
});

describe('block64 functions', () => {
    test('it can read 64 bit blocks as left 32 bits and right 32 bits', () => {
        expect(util.readBlock64('abcdefgh', 0)).toMatchObject({left: 1633837924, right: 1701209960 });
    })
    test('it can write a 64 bit block { left, right } as a string', () => {
        expect(util.block64ToString({left: 1633837924, right: 1701209960 })).toBe('abcdefgh');
    })
});
